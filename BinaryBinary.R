BinaryBinary = function(Cobs, Cmis, covType, X, Y, savePosterior = FALSE,
  ...
  for (i in 2 : nScans) {
    ...
    whichColY = c(which(alphaYpost[i-1,]==1), which(alphaYpost[i-1,1:M]==1) + P)
    meanZy = cbind(rep(1,N), X, X*StudyInd, Cupdate[,whichColY]) %*% 
      thetaYpost[i-1,c(1,2,3,whichColY+3)]
    
    Zy[Y==1] = rtruncnorm(sum(Y==1), a=0, mean = meanZy[Y==1], sd=1)
    Zy[Y==0] = rtruncnorm(sum(Y==0), b=0, mean = meanZy[Y==0], sd=1)
    ...
    if (length(wYprop) >= 1) {
      fitYprop = glm(Y ~ X + I(X*StudyInd) + Cupdate[,wYprop], family=binomial(link="probit"))
    } else fitYprop = glm(Y ~ X + I(X*StudyInd), family=binomial(link="probit"))
    
    if (length(wYpost) >= 1) {
      fitYpost = glm(Y ~ X + I(X*StudyInd) + Cupdate[,wYpost], family=binomial(link="probit"))
    } else fitYpost = glm(Y ~ X + I(X*StudyInd), family=binomial(link="probit"))
    ...
    muY = solve((t(Wy)%*%Wy) + (diag(dim(Wy)[2])*sigmaYpost[i-1]/k)) %*% t(Wy) %*% Zy
    ...
    for (p in (M+1) : P) {
      ...
      if (covType[p]=="Binary") {
        ...
        if (alphaYpost[i,p] == 1) {
          muYcomp = Zy - thetaYpost[i,2]*X - thetaYpost[i,3]*X*StudyInd - rep(thetaYpost[i,1], length(X))
          for (num.p in 1:(P+M)) {
    ...
    acepost[i] = mean(pnorm((cbind(rep(1, N-n), rep(1,N-n), rep(0,N-n), Cupdate[(n+1):N,]) %*% thetaYpost[i,])) - 
                        pnorm((cbind(rep(1, N-n), rep(0, N-n), rep(0,N-n), Cupdate[(n+1):N,]) %*% thetaYpost[i,])))
  }
  ...
  if (savePosterior == FALSE) {
    l = list(InclusionProbX = apply(alphaXpost[keep,], 2, mean, na.rm=TRUE),
             ...
             ATE = mean(acepost[keep], na.rm=TRUE),
             ATE_CI = quantile(acepost[keep], c(.025, .975), na.rm=TRUE))
  } else {
